...
class MeterFileProcessor(private val objectMapper: CustomObjectMapper) {
    ...
    private fun MeterReading.toReport(): MeterReport {
        val rawData: MutableMap<Long, BigDecimal> = mutableMapOf()
        ...
        intervalReadings.forEach {
            ...
            for (second in startOfReading until endOfReading) {
                ...
                val usage = rawData[startOfHour] ?: ZERO
                rawData[startOfHour] = usage.plus(usagePerSecond)
            }
        }

        val usageSum = rawData.values.reduce(BigDecimal::add).roundTwoDecimals()
        val priceSum = usageSum.multiply(meterInfo.unitPrice).applyFlow(meterInfo.flowDirection).roundTwoDecimals()
        val hourlyData = rawData.toReportFormat(meterInfo.unitPrice.applyFlow(meterInfo.flowDirection))
